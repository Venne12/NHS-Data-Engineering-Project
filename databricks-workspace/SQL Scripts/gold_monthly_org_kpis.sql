...
/* Create the Gold Organisation-Level KPI table */
CREATE OR REPLACE TABLE gold.monthly_org_kpis
USING DELTA
AS
SELECT
    period,
...
/* Validate the table */
SELECT *
FROM gold.monthly_org_kpis
ORDER BY period, total_ae_attendances DESC;

/* Create an external Gold table from existing one */
CREATE TABLE gold.monthly_org_kpis_ext
USING DELTA
LOCATION 'abfss://data@nhsdatalakevenz.dfs.core.windows.net/curated/gold/monthly_org_kpis'
AS
SELECT *
FROM gold.monthly_org_kpis;

/* Validate tables */
SELECT COUNT(*) FROM gold.monthly_org_kpis_ext;
SELECT COUNT(*) FROM gold.monthly_org_kpis;

/* Drop the original table and Rename the external table to original name */
DROP TABLE gold.monthly_org_kpis;

ALTER TABLE gold.monthly_org_kpis_ext
RENAME TO gold.monthly_org_kpis;
